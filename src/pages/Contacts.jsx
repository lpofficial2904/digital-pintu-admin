...
export default function Contacts() {
  ...
  const load = async () => {
    try {
      const d = await getAdminContacts();
      setItems(d.contacts || []);
    } catch (e) {
  ...
  const data = useMemo(
    () =>
      items.filter(
        (x) =>
          `${x.name} ${x.email} ${x.phone} ${x.service} ${x.message}`
            .toLowerCase()
            .includes(q.toLowerCase()) &&
          (filter === "all" || (x.status || "New") === filter),
      ),
    [items, q, filter],
  );
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-white/10 bg-slate-900/80 p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="text-2xl font-semibold">Inbox and inquiries</h2>
          </div>
          <div className="flex gap-2">
            <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-slate-950 p-2">
              <Search size={16} />
              <input
                value={q}
            ...
            <select
              ...
              className="rounded-xl border border-white/10 bg-slate-950 p-2"
            >
      ...
      {loading ? (
        ...
        <>
          ...
          >
            {data.slice((page - 1) * per, page * per).map((x) => (
              <tr key={x._id}>
                <td className="px-4 py-3 font-medium">{x.name}</td>
                <td className="px-4 py-3 text-slate-400">{x.email}</td>
                <td className="px-4 py-3 text-slate-400">{x.phone || "—"}</td>
                <td className="px-4 py-3">{x.subject || x.service || "—"}</td>
                <td className="max-w-48 truncate px-4 py-3 text-slate-400">
                  {x.message}
                </td>
                <td className="px-4 py-3">
                  <select
                    value={x.status || "New"}
                    onChange={(e) => update(x._id, { status: e.target.value })}
                    className="rounded-lg border border-white/10 bg-slate-950 p-1 text-xs"
                  >
                    {statuses.map((s) => (
      ...
      >
        {edit && (
          ...
          >
            ...
            <input
              name="subject"
              defaultValue={edit.subject || edit.service || ""}
              placeholder="Subject"
              className="w-full rounded-xl border border-white/10 bg-slate-950 p-3"
            />
            <textarea
